...
const registerController = async (req, res) => {
  const { firstname, lastname, gender, password, confirmpassword, email } =
    req.body;
  if (
    !firstname ||
    !lastname ||
    ...
    return res
      .status(409)
      .json({
        msg: "firstname,lastname,gender,password,confirmPassword,email must be provided",
      });
  }
  ...
  if (confirmpassword !== password) {
    return res.status(409).json({ msg: "Passwords must match" });
  }
  ...
  try {
    ...
    if (duplicate) {
      return res
        .status(409)
        .json({ msg: `user with email of ${email} already exists` });
    }
